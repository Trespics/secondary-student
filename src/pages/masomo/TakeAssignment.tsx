...
const TakeAssignment = () => {
  ...
  useEffect(() => {
    const fetchAssignment = async () => {
      try {
        ...
        setTimeLeft((found.time_limit_minutes || 30) * 60);
      } catch (err) {
  ...
  useEffect(() => {
    if (timeLeft <= 0 || isCompleted || loading || !assignment) return;
  ...
  }, [timeLeft, isCompleted, loading, assignment]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data } = await api.post("/student/submissions", {
        assignment_id: id,
        answers: answers
      });
      setResult(data);
        ...
        <div className="max-w-2xl mx-auto py-12 px-4">
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-500 text-lg">Great job on finishing the assignment.</p>
            </div>
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 font-medium">Question {currentQuestionIndex + 1} of {assignment.questions.length}</p>
          </div>
