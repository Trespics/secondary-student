...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await api.post("/auth/forgot-password", { email, clientUrl: window.location.origin });
      setIsSubmitted(true);
    } catch (err: any) {
  ...
  const boxStyle = { width: "100%", maxWidth: "420px", background: "rgba(30,41,59,0.95)", backdropFilter: "blur(10px)", borderRadius: "24px", padding: "2.5rem", boxShadow: "0 25px 50px rgba(0,0,0,0.25), 0 0 0 1px rgba(255,255,255,0.05)" };
  const btnStyle = { width: "100%", padding: "0.9rem", background: isLoading ? "#475569" : "linear-gradient(135deg,#3b82f6,#6366f1)", color: "white", border: "none", borderRadius: "14px", fontSize: "0.95rem", fontWeight: 600 as const, cursor: isLoading ? "not-allowed" as const : "pointer" as const, boxShadow: "0 4px 15px rgba(59,130,246,0.3)", display: "flex" as const, alignItems: "center" as const, justifyContent: "center" as const, gap: "0.5rem" };
  const inputStyle = { width: "100%", padding: "0.875rem 1rem 0.875rem 3rem", background: "#1e293b", border: "2px solid #334155", borderRadius: "14px", fontSize: "0.95rem", color: "#e2e8f0", outline: "none", fontFamily: "inherit", boxSizing: "border-box" as const };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0f172a 0%,#1e1b4b 50%,#0f172a 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem" }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={boxStyle}>
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          ...
          <p style={{ color: "#94a3b8", fontSize: "0.9rem" }}>
            {isSubmitted ? "We've sent a reset link to your email." : "Enter your email to receive a password reset link."}
          </p>
        </div>
        {isSubmitted ? (
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            <div>
              ...
              <div style={{ position: "relative" }}>
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="student@school.com" style={inputStyle} />
              </div>
            </div>
